...
class SQLighter:
	def __init__(self, db):
		...
		self.cursor.execute("CREATE TABLE IF NOT EXISTS users(user_id INT PRIMARY KEY, account_id INT, token TEXT, accounts TEXT);")
		self.cursor.execute("CREATE TABLE IF NOT EXISTS accounts(account_id INT PRIMARY KEY,url TEXT, login TEXT, password TEXT, school INT);")
	...
	def add_user(self, user_id, account_id = 0, token = secrets.token_hex(), accounts = '[]'):
		with self.connection:
			return self.cursor.execute('INSERT INTO `users` (`user_id`, `account_id`, `token`, `accounts`) VALUES(?,?,?,?)', (user_id, account_id, token, accounts))

	def add_account(self, account_id, url, login, password, school):
		with self.connection:
			if not self.account_exists(account_id):
				return self.cursor.execute('INSERT INTO `accounts` (`account_id`, `url`, `login`, `password`, `school`) VALUES(?,?,?,?,?)', (account_id, url, login, password, school))
			else:
				print('Аккаунт зареган')
